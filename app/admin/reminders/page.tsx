...
export default function RemindersPage() {
  ...
  const handleSendReviewRequests = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setReviewResult(`✅ Отправлено запросов на отзывы: ${data.processed}`)
      } else {
        setReviewResult(`❌ Ошибка: ${data.error || 'Неизвестная ошибка'}`)
      }
    } catch (error) {
      setReviewResult('❌ Ошибка при отправке запросов на отзывы')
    } finally {
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mt-6">
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-600 mb-4">
                Система автоматически найдет все завершенные записи (статус "Завершена") и отправит запрос на отзыв через WhatsApp через 1 час после завершения сеанса. Каждый запрос отправляется только один раз.
              </p>
            </div>
            ...
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
              ...
              <ul className="text-sm text-purple-800 space-y-1 list-disc list-inside">
                <li>Система проверяет все записи со статусом "Завершена"</li>
                <li>Отправляет запрос на отзыв через 1 час после завершения сеанса</li>
                <li>Каждый запрос отправляется только один раз</li>
                <li>Требуется настроенный WhatsApp в настройках</li>
                <li>Шаблон сообщения можно настроить в разделе "Настройки"</li>
              </ul>
            </div>
            ...
            >
              {sendingReviews ? (
                ...
                <>
                  ...
                  Отправить запросы на отзывы сейчас
                </>
              )}
            </button>
          </div>
